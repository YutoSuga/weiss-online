...
手札はGameStateの`hand.length`に応じてRendererがカード要素を動的生成し、
`1`から実枚数まで連続した`data-index`を設定する。7を表示上限としない。
表示枠は7枚分を最小幅、12枚分を既存盤面での安全な最大幅とし、
13枚目以降もDOM生成したうえで手札枠内を横スクロールする。
...
舞台の data-index は、画面上の左からではなく、各カード所有者から見た左→右の順番とする。
