...
GameEngineはBRAINSTORM_REVEALの移動済みinstanceをACT contextで追跡し、山札0枚なら`resolveRuleCheck()`から既存Refresh/Penaltyへ割り込む。Groupの最小条件評価、結果参照、SEARCH_DECK開始・toggle・confirm、同一instanceのHand移動、Deck shuffleを担当する。ControllerやRendererはfilterや移動を判定しない。

## F-5D-2のカード移動

PRINTED AUTO①のStock topはPlayerの`stock.at(-1)`とし、Stock→Waiting Roomおよび選択後のWaiting Room→Stockはいずれも`GameEngine.moveCard()`へ委譲する。このためcollection、index、Card locationを同時に更新し、各移動後に通常の`CARD_MOVED`、Trigger Detection、Pending登録を行う。DEVの「相手山札TOP→相手Stock」も既存`moveDeckCardToStock("opponent")`を利用する。
