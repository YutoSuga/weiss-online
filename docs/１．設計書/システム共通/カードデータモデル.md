...
カードデータを「そのカードが何であるか」を表すimmutableな`CardMaster`、「そのカードに何が書かれているか」を表すimmutableな`CardAbility`、対戦中のmutableな1枚である`Card`に分離する。F-3Dまで実装済みで、次PhaseはF-4（ACT Ability v1 + 代表的な実カード）である。

```mermaid
classDiagram
  CardMaster "1" *-- "0..*" CardAbility : abilities（直接包含）
  CardMaster "1" <-- "0..*" Card : masterId / resolved master
  class CardMaster { id; imageUrl; triggerIcons; traits; text; abilities }
...
`CardMaster.imageUrl`はカード画像の固定情報で、空でないURL文字列または`null`を保持する。CardMasterの他の情報と同様にimmutableであり、`Card`は`card.imageUrl` getterで委譲する。runtime serializationには複製しない。
...
```text
CardMaster（固定情報 / imageUrl / CardAbility[]）
  ↓ CardMasterへの委譲getter
Card（固定情報の表示窓口 + runtime情報）
  ↓
Renderer
  ↓
対戦画面 / 右上カード詳細
...
Rendererは基本的に`Card`だけを受け取り、`CardMasterRegistry`を直接参照しない。固定情報はCardの委譲getter、現在Power/Soul・位置・表裏等はCard自身から取得する。画像は`CardMaster.imageUrl → Card.imageUrl → Renderer`、能力表示は`CardMaster.abilities → Card.abilities → CardAbility.type / text → Renderer`の経路を取る。Rendererはconditions / costs / effectsを解釈せず、legacyな`CardMaster.text`も対戦詳細の能力本文に使用しない。

PowerとSoulはcurrent値を主表示し、base値と異なる場合だけ`2000（元1500）`、`2（元1）`の形式で印刷値を併記する。画像・名称を含む固定情報はvisibility判定を通過したカードにのみ表示する。

## Phase F-5D-2 PRINTED AUTO

PRINTED AUTOはCardMasterの`abilities`に構造化`CardAbility`として定義し、Card instanceは従来どおり`masterId`経由で参照する。`CHA/W40-026SP`は同一の`CARD_MOVED: SELF / HAND → STAGE`を持つ2定義を保持する。能力IDはCardMaster内で別々であり、Trigger Detectorは一致した定義ごとにPendingを生成する。能力定義や使用可否をCard instanceへ複製しない。
