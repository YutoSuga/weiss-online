...
本書は、対戦ドメインの主要なEntity / Data Objectと、それらを操作・表示するService / Componentの責務境界を一覧にする。「実装済み」は現在のリポジトリに対応するクラスが存在することを示す。F-3Aまで実装済みで、CardAbilityはF-3B予定である。詳細なカードデータ仕様は[カードデータモデル](カードデータモデル.md)を正本とする。

## Entity / Data Object

| 物理名 | 論理名 | 分類 | 主な役割 | mutable / immutable | 主な保持先・管理主体 | 現在の実装状況 |
| --- | --- | --- | --- | --- | --- | --- |
| `CardMaster` | カードマスター | Data Object | 「そのカードが何であるか」を表す固定情報。`id`と実カード番号`cardNumber`は別概念 | immutable | F-3Aの`CardMasterRegistry`がメモリ上で管理 | **F-3A実装済み** |
