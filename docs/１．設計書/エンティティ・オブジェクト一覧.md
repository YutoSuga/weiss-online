...
本書は、対戦ドメインの主要なEntity / Data Objectと、それらを操作・表示するService / Componentの責務境界を一覧にする。「実装済み」は現在のリポジトリに対応するクラスが存在することを示す。F-3Cまで実装済みである。詳細なカードデータ仕様は[カードデータモデル](カードデータモデル.md)を正本とする。
...
| `DeckDefinition` | デッキ定義 | Data Object | `masterId + count`で対戦開始前のデッキ構成を表す | immutableとして扱う | 将来のデッキ構築・対戦準備境界 | **F-3C実装済み**。構築ルール判定は持たない |
...
| `CardMasterLoader` | カードマスターローダー | Data Service | データソース取得、基本形式検証、CardMaster生成・Registry登録 | stateless | 対戦準備・カード一覧等から利用可能 | **F-3C実装済み** |
| `DeckDefinitionLoader` | デッキ定義ローダー / 展開 | Data Service | DeckDefinition取得とRegistryを用いたCard instance生成 | stateless | 対戦準備境界 | **F-3C実装済み** |
...
flowchart LR
  ...
  Master -->|abilities 0..*| Ability["CardAbility<br/>immutableな能力定義"]
...
F-3Dでは`CardMaster.imageUrl`を空でないstringまたは`null`の固定情報として追加し、Cardが`imageUrl`を含む固定情報と`abilities`を委譲getterで公開する。RendererはCardを入口にしてこれらを表示し、CardMasterRegistryを直接参照しない。Cardの`currentPower` / `currentSoul`はruntime値として引き続きCard自身が保持する。
...
- `Player.resolution`: player別のPUBLICな一時解決領域。
- `CardSelectionView`: MulliganとDeck Searchが共有する選択状態表示Component。ゲームルールは持たない。
- `SEARCH_DECK Process`: 選択状態をcontextへ持つACTの子Process。
- ACT `effectResults`: Effect.id keyの一時結果。GameStateへ恒久保存しない。
