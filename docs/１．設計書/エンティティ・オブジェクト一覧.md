...
本書は、対戦ドメインの主要なEntity / Data Objectと、それらを操作・表示するService / Componentの責務境界を一覧にする。「実装済み」は現在のリポジトリに対応するクラスが存在することを示し、F-3A以降の設計を実装済みとは扱わない。詳細なカードデータ仕様は[カードデータモデル](カードデータモデル.md)を正本とする。

## Entity / Data Object

| 物理名 | 論理名 | 分類 | 主な役割 | mutable / immutable | 主な保持先・管理主体 | 現在の実装状況 |
| --- | --- | --- | --- | --- | --- | --- |
| `CardMaster` | カードマスター | Data Object | 「そのカードが何であるか」を表す固定情報。`id`と実カード番号`cardNumber`は別概念 | immutable | F-3Aの`CardMasterRegistry`がメモリ上で管理 | **F-3A予定**。未実装 |
| `CardAbility` | カード能力定義 | Data Object | 「そのカードに何が書かれているか」を表す能力定義 | immutable | `CardMaster.abilities[]` | **F-3B予定**。F-3Aでは実装しない |
| `Card` | 対戦中カードインスタンス | Entity | 「その対戦中、その1枚が現在どうなっているか」を表す | mutable | `Deck`または`Player`の各Zone。`GameState`から到達可能 | **実装済み**。現在は固定情報も直接保持。`instanceId` / `masterId`分離と互換getter化は**F-3A予定** |
...
| 物理名 | 論理名 | 分類 | 主な役割 | mutable / immutable | 主な保持先・管理主体 | 現在の実装状況 |
| --- | --- | --- | --- | --- | --- | --- |
| `CardMasterRegistry` | カードマスターレジストリ | In-memory Registry | 読み込み済み`CardMaster`を`masterId`で登録・取得する | Registryの登録集合はmutable、登録するmasterはimmutable | アプリケーションの対戦準備境界で構築 | **F-3A予定**。未実装。永続ストレージではない |
...
flowchart LR
  ...
  Master -->|abilities 0..*| Ability["CardAbility<br/>immutableな能力定義 / F-3B"]
...
## F-3Aで導入予定のRegistry境界

`CardMasterRegistry`の想定APIは`register(master)`、`get(masterId)`、`has(masterId)`、`getAll()`とする。これは読み込み済みmasterを参照するためのメモリ上のRegistryであり、永続ストレージやRepositoryではない。ブラウザ再読み込み後は必要なmasterを取得して再構築する。
