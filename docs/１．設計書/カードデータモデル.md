...
本書は、カード種類の固定データ、対戦中のCard instance、CardAbilityを分離・実装するための設計基準を定義する。`CardMaster`、`CardMasterRegistry`、Cardとの分離はF-3Aで実装済みである。`CardAbility`、Ability Engine、正式なCardMasterデータディレクトリは未実装であり、次PhaseはF-3Bとする。
...
Phase F-2BのLevel・Color・Cost条件を実画面で確認するため、`client/data/test-cards.json` に固定情報だけを持つ暫定カード定義を置く。F-3Aの開発用ローダーは定義を検証し、`trigger`を`triggers`へ変換したCardMasterをRegistryへ登録する。そのRegistryをCard constructorへ注入し、self / opponentそれぞれに独立したCard instanceからDeckを生成する。

JSONの`id`は暫定的な定義識別子であり、F-3A adapterでは`CardMaster.id` / `Card.masterId`として扱う。複数枚を生成する際はowner・定義ID・copy番号から一意な`Card.instanceId`を作り、互換getterの`Card.id`も同じ値を返す。このJSON schemaを最終仕様とはせず、F-3CでCardMaster JSON schemaへ移行する。

## F-3A以前のCard実装

F-2時点の`client/js/models/card.js`の`Card`は、1枚の物理カードの状態に加えて、固定情報も同居して保持していた。
...
- `id`はCard instanceを識別するIDだった。F-3A以降は`instanceId`の互換getterである。
...
F-3Aでは以下の互換getterを残して内部構造だけを移行した。

## CardMaster とCard instance分離（F-3A実装済み）

カード種類の固定データをCardMasterとして独立させ、Card instanceは`masterId`で参照する。
...
`CardMaster.id`はweiss-online内部で参照に使うMaster ID、`cardNumber`は実カードに印刷されたカード番号であり、別概念とする。暫定データに実カード番号がないため`cardNumber`は`null`である。CardMaster本体と、コピー済みの`triggers`、`traits`はfreezeし、対戦中に書き換えない。`abilities`プロパティとその実体はF-3Bで導入し、F-3Aでは実装しない。
...
### CardMasterRegistry（F-3A実装済み）

`CardMasterRegistry`は、読み込み済みのimmutableなCardMasterを`masterId`から取得するメモリ上のRegistryである。APIは次のとおりとする。

- `register(master)`: CardMaster以外および重複IDを拒否する。
- `get(masterId)`: 未登録IDは原因を示すErrorをthrowする。
- `has(masterId)`
- `getAll()`: Registry内部を変更できないfreeze済み配列コピーを返す。

Registryは永続ストレージではない。ブラウザを再読み込みするとメモリ上のRegistryは失われ、必要なCardMasterを再読み込みして再構築する。将来、取得元がJSON、API、DBへ変更されても、CardとGameEngineはその取得元や保存schemaへ直接依存しない。

対戦準備時にRegistryからmasterを解決してCard instanceを生成し、Cardは解決済みmaster参照を通じて互換getterを提供する。これによりGameEngineはRegistryを都度検索せず、Cardの公開APIだけに依存できる。

### F-3Aで確定したserialization

`Card.toJSON()`は`instanceId`, `masterId`, owner、位置・表示・現在Power/Soulだけを保存し、CardMaster固定情報を埋め込まない。`Card.fromJSON(data, masterRegistry)`は注入されたRegistryでMasterを解決し、未登録IDを拒否する。`face`と`visibilityOverride`の通常値は引き続き`null`である。

CardMasterの正式なロード・キャッシュ・検証方法、能力テキスト`text`と将来のAbility / flavor textの関係、画像情報は後続Phaseで確定する。F-3Aの`text`はlegacy / transitionalな固定情報であり、flavor textへ読み替えない。

CardMasterは対戦Card instanceとは分離し、Repository内データとして管理する方向である。保存場所はF-3Cで決定し、将来JSON、DB、APIへ移行可能な読み出し境界を設ける。

DeckDefinitionはCardMaster全文ではなく`masterId + count`を保持し、対戦開始時にCard instanceへ展開する。画面フローと外部Import境界は[Application Flow / Deck設計](アプリケーションフロー.md)を正本とする。
