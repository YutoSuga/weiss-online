...
本書は、カード種類の固定データ、対戦中のCard instance、CardAbilityを分離・実装するための設計基準を定義する。現時点では`CardMaster`、`CardMasterRegistry`、`CardAbility`、Ability Engine、正式なCardMasterデータディレクトリはいずれも未実装である。本書はF-3A以降の実装計画であり、本文書整理だけでF-3Aを完了扱いしない。
...
`CardMaster.id`はweiss-online内部で参照に使うMaster ID、`cardNumber`は実カードに印刷されたカード番号であり、別概念とする。CardMasterと、その配列プロパティである`triggers`、`traits`、`abilities`はimmutableとし、対戦中に書き換えない。`abilities`の実体はF-3Bで導入し、F-3Aでは実装しない。
...
### CardMasterRegistry（F-3A予定）

`CardMasterRegistry`は、読み込み済みのimmutableなCardMasterを`masterId`から取得するメモリ上のRegistryとする。想定APIは次のとおりとする。
...
- CardMasterRegistryの具体的な所有者、ロード完了タイミング、重複ID時の扱い
- `trigger`（現行Cardの配列）とCardMasterの`triggers`の対応
- 能力テキスト`text`と、将来フレーバーテキストを追加する場合の保存形式
