...
`canSelectCardForMain()`は自分のターンのMAIN / WAITING_INPUT中に自分のHand Cardを詳細確認のため選択できるかを表し、cardTypeやプレイ条件は判定しない。CHARACTERのStage play可否は`canPlayCharacterToStage()` / `getCharacterPlayDisabledReason()`がLevel・Color・Costを含めて別途判定する。したがってプレイ不可のCHARACTERも選択と詳細表示ができる。CLIMAXも選択と詳細表示はできるが、`getMainDestinationCandidates()`はDestinationを返さず、`PLAY_CHARACTER`は開始しない。MAIN以外や相手ターンの選択へは拡張しない。
...
| 選択肢 | 結果 |
| --- | --- |
| 置き換える（圧殺） | 既存カードをWAITING_ROOMへ送り、HANDカードを配置する。 |
...
`UI操作.md`は、本書と同じクリック/タップによる「カード選択 → Action選択 / Destination選択」の方針へ更新済みである。全Phase共通の詳細なAction UIは同書を正本とし、MAIN固有の操作だけを本書で補足する。
